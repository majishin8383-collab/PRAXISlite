(() => {
  try {
    // Mark as loaded (index.html will check this)
    ...
    // Render crash visibly in the UI
    ...
    if (t) t.textContent = "App crashed at startup";
    if (s) s.textContent = "A JS error occurred. The details are shown below.";
    ...
    if (b) {
      ...
      b.innerHTML = `
        <div class="block">
          <h3>Crash details</h3>
          <p style="font-family: var(--mono); white-space: pre-wrap; line-height: 1.35;">${msg}</p>
          <div class="hr"></div>
          <p>Most common causes: missing element IDs, file path mismatch, or a syntax error in app.js.</p>
        </div>
      `;
    }
  }
